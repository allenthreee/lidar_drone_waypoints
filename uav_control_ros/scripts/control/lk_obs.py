...
class lk_obs:
    def __init__(self, name: str):
        ...
        self.beta2 = np.array(_p['beta2']).astype(float)
        self.beta3 = np.array(_p['beta3']).astype(float)
        self.beta1 = np.sqrt(self.beta2 * self.beta3)

        self.Td = np.array(_p['Td']).astype(float)
        ...
        self.rec_xi = np.empty(shape=[0, self.dim], dtype=float)
        self.rec_var_theta = np.empty(shape=[0, self.dim], dtype=float)
        self.rec_d_var_theta = np.empty(shape=[0, self.dim], dtype=float)
        self.rec_d_sigma = np.empty(shape=[0, self.dim], dtype=float)
        self.rec_sigma = np.empty(shape=[0, self.dim], dtype=float)
        self.rec_delta = np.empty(shape=[0, self.dim], dtype=float)
        '''data record'''
